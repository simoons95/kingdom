            ...
            try:  # check message correctness
                ...
                for player_line in message.partition(ENDING)[0].split("<br/>"):
                    ...
                    if search:
                        ...
                        assert int(search.group(2)) == 9 + 3 * (delta // 10), "Prévenir @simoons:528629 svp."
                    check = ps.health == 1 or ps.health == new_health(p.states[max(p.states)].health,
                                                                      search and int(search.group(1)),
                                                                      search and int(search.group(2)))
...
def get_map_histo(muxxu_groups, players):
    for muxxu_group in muxxu_groups:
        ...
        for entry in histo.split('</li><li>')[::-1]:
            ...
            if date in player.states:
                if player.states[date].health != 0:
    ...
    for player in sorted(players.values(), key=lambda x: x.name):
        if now in player.states:
            ...
            if 0 < last_state.health < SANTE.index("Excellente santé"):
                ...
                de6 = " + ({{d100}} <= {})".format(9 + 3 * (days_in//10))
            else:
                de6 = ""
            yield "@{}:{}-{}-{}.{:02d} : {}{}".format(
...
def main():
    ...
    now = datetime.datetime.today() + datetime.timedelta(days=add)
...
# def extract_players_age(ranking_pages):
...
#                         id2player[player] = line.partition('</a></td>')[0][::-1].partition(" ")[0][::-1] + " ({})".format(player)
...
#                     warnings.append("Problem with {}, {}.{} to {}.{} in {} hours (between {} and {})".format(id2player[player2], old_age//12, old_age%12, age2//12, age2%12, hour2 - hour1, hour1, hour2))
#     if max_factor > 0:
#         print("Maximum factor obtained by {} with {} between {} and {}, with {}.{} to {}.{} in {}d {}h".format(
#             id2player[max_player], max_factor, max_hour1, max_hour2, max_age1//12, max_age1%12, max_age2//12, max_age2%12, (max_hour2-max_hour1)//24, (max_hour2-max_hour1)%24))
#         [print("{} got maxi {}".format(id2player[player], max_fac)) for player, max_fac in sorted(max_factor_player.items(), key=lambda x: x[1])]
#
